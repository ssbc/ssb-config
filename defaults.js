...
function setDefaults (name, config) {
  ...
  if (!config.connections.incoming) {
    config.connections.incoming = {
      net: [{ host: config.host, port: config.port, scope: "public", "transform": "shs" }],
      ws: [{ host: config.host, port: config.ws.port, scope: "device", "transform": "shs" }]
    }
  }
